...
<?php
	echo anchor('History_ctrl/generate_pdf', 'Generate PDF Report');
...
<div class="container-fluid">
    ...
    <form style="padding-right: 0px;margin-right: 0px;margin-left: 0px;" method="post" action="../addRejDetails">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-2"><input type="text" class="form-control" name="search" id="search"  autofocus placeholder="Enter the Donor's Barcode" /></div>
        ...
        <!-- <?php foreach($user as $row):?><?=$row->blood_gr;?><?php endforeach;?> -->
            ...
            <div class="form-group col-md-3" style="margin-right: 0px;"><label>Camp :</label> <input style="border:none;" id="" readonly/></div>
        </div>

        <hr>

        <div class="form-row" style="margin-top : 1%";>
            <div class="form-group col-md-4" style="margin-right: 0px;"><label>Email :</label> <input style="border:none;" id="email" size="40" readonly/></div>
            <div class="form-group col-md-4" style="margin-right: 0px;"><label>Age :</label> <input style="border:none;" id="age" readonly/></div>
            <div class="form-group col-md-4" style="margin-right: 0px;"><label>Address :</label> <input style="border:none;" size="50" id="address" readonly/></div>
            <div class="form-group col-md-4" style="margin-right: 0px;"><label>Pincode :</label> <input style="border:none;" id="pincode" readonly/></div>
            <div class="form-group col-md-4" style="margin-right: 0px;"><label>Date of Birth :</label> <input style="border:none;" id="dob" readonly/></div>
            <div class="form-group col-md-4" style="margin-right: 0px;"><label>Occupation :</label> <input style="border:none;" id="occupation" readonly/></div>
        </div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Choose Donor</h4>
        ...
        <table class="table table-hover table-bordered results">
            ...
            <tbody>
                <?php $i=1; foreach($user as $row):?>
                    <tr onclick="enterdetails('<?=$row->id?>', '<?=$row->name?>', '<?=$row->blood_gr?>', '<?=$row->sex?>', '<?=$row->phone_no?>', '<?=$row->email?>', '<?=$row->age?>', '<?=$row->address?>', '<?=$row->pin?>', '<?=$row->dob?>', '<?=$row->occ?>')">
                        <td><?=$i?></td>
                        <td><?=$row->id;?></td>
                        <td><?=$row->name;?></td>
                        <td><?=$row->date;?></td>
...
function enterdetails(id,name,bgrp,sex,phno,email,age,address,pin,dob,occupation){
...
<script>
  function searchip(){
    ...
    xhttp.open("GET", "getDonorInfo?id="+search_ip, true);
    xhttp.send();
  }
